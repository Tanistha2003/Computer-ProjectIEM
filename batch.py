...
def viewStudents(batch_id):
    ...
    for student in students:
        print(student)
def viewCourses(batch_id2):
    with open("batch.csv", "r", newline = "\n") as f:
        csv_reader = list(csv.reader(f, delimiter=","))
    check = 0
    courses = []
    for i in range(0, len(csv_reader)):
        if(csv_reader[i][0] == batch_id2):
            check = 1
    ...
    for course in courses:
        print(course)
def viewPerformance(batch_id3):
    with open("batch.csv", "r", newline = "\n") as f:
        csv_reader = list(csv.reader(f, delimiter=","))
    check = 0
    students = []
    for i in range(0, len(csv_reader)):
        if(csv_reader[i][0] == batch_id3):
            check = 1
            students = csv_reader[i][4].split(":")
            break
    if(check == 0):
    ...
    for student in students:
        ...
        print()
def pieChart(batch_id4):
    with open("batch.csv", "r", newline = "\n") as f:
        csv_reader = list(csv.reader(f, delimiter=","))
    check = 0
    students = []
    for i in range(0, len(csv_reader)):
        if(csv_reader[i][0] == batch_id4):
            check = 1
            students = csv_reader[i][4].split(":")
            break
    if(check == 0):
    ...
    pyplot.show()
batch_name=input("Enter batch name to add")
batch_id=input("Enter batch_id to view")
batch_id2=input("Enter batch id you want to view courses of")
batch_id3=input("Enter batch id you want to see performance about")
batch_id4=input("Enter batch id you want to see pie chart of")